...
export class StoreComponent implements OnInit {
  ...
  private async payFarmeables(value: CheckoutFarmeable){
    // var farmeable = this.farmeableSVC.getFarmeableById(value.farmeable)
    var farmeable = await this.farmeableSVC.getFarmeableById(value.farmeable)
    console.log(farmeable.name);
    if(value != null && farmeable){
      ...
      //Restamos el dinero
        ...
        if(this.moneySVC.getMoney.value >= this.price){
          ...
          console.log("no se puede pafgar")
        }
    }
